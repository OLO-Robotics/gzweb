...
import System, { Emitter, SpriteRenderer, Body, Position, BoxZone, Life, Rate, Radius, VectorVelocity, Span, Scale } from 'three-nebula';
 ...
 */
function getDescendants(obj, array = []) {
    obj.traverse((child) => {
                ...
                switch (key) {
                    case 'diffuse':
                        ...
                        if (parameter.texture)
                            material.map = getTexture(parameter.texture, LinearEncoding);
                        break;
                    case 'specular':
                    ...
                    case 'ambient':
                        if (parameter.texture)
                            material.lightMap = getTexture(parameter.texture, LinearEncoding);
                        break;
                    case 'shininess':
                    ...
                    case 'emission':
                        ...
                        if (parameter.texture)
                            material.emissiveMap = getTexture(parameter.texture, LinearEncoding);
                        break;
                }
            }
            // Modified by German Mas.
            ...
            for (let i = 0, l = xml.childNodes.length; i < l; i++) {
                ...
                switch (child.nodeName) {
                    case 'color':
                        ...
                        data.color = new Color$1().fromArray(array).convertSRGBToLinear();
                        break;
                    case 'falloff_angle':
...
// channel.
class Color extends THREE$1.Color {
    constructor(r, g, b, a) {
        super(r, g, b);
        this.a = 1.0;
        if (a) {
                ...
                for (let mi = 0, miLen = materials.length; mi < miLen; mi++) {
                    ...
                    if (this.materials !== null) {
                        ...
                        if (isLine && material && !(material instanceof LineBasicMaterial)) {
                            ...
                            Material$1.prototype.copy.call(materialLine, material);
                            materialLine.color.copy(material.color);
                            material = materialLine;
                        }
                        else if (isPoints && material && !(material instanceof PointsMaterial)) {
                            const materialPoints = new PointsMaterial({ size: 10, sizeAttenuation: false });
                            Material$1.prototype.copy.call(materialPoints, material);
    ...
    createMaterial_(materialName) {
        ...
        for (const prop in mat) {
            ...
            switch (prop.toLowerCase()) {
                ...
                case 'kd':
                    // Diffuse color (color under white light) using RGB values
                    params.color = new Color$1().fromArray(value).convertSRGBToLinear();
                    break;
                case 'ks':
                    // Specular color (color when light is reflected from shiny surface) using RGB values
                    params.specular = new Color$1().fromArray(value).convertSRGBToLinear();
                    break;
                case 'ke':
                    // Emissive using RGB values
                    params.emissive = new Color$1().fromArray(value).convertSRGBToLinear();
                    break;
                case 'map_kd':
                    // Diffuse texture map
...
class GzObjLoader {
     ...
     */
    constructor(_scene, _uri, _submesh, _centerSubmesh, _findResourceCb, _onLoad, _onError, _files) {
        this.uri = '';
        this.baseUrl = '';
        this.files = [];
     ...
     */
    applyMaterial(_mtlCreator) {
        let allChildren = [];
        getDescendants(this.container, allChildren);
        for (let j = 0; j < allChildren.length; ++j) {
            let child = allChildren[j];
    ...
    onObjLoaded(_container) {
        ...
        if (!this.usingRawFiles) {
            for (var i = 0; i < this.container.materialLibraries.length; ++i) {
                // Load raw .mtl file
                let mtlPath = this.container.materialLibraries[i];
        ...
        this.update = function () {
            ...
            return function update() {
                ...
                if (Math.abs(sphericalDelta.phi - 0) > 0.001 ||
                    ...
                    scope.object.position.sub(scope.target);
                    scope.object.updateMatrix();
                    rotateAroundWorldAxis(scope.object, new Vector3(0, 0, 1), spherical.theta - oldTheta);
                    let localPitch = new Vector3(1, 0, 0);
                    localPitch.applyQuaternion(scope.object.quaternion);
...
const FUEL_HOST = 'fuel.gazebosim.org';
const FUEL_VERSION = '1.0';
const IGN_FUEL_HOST = 'fuel.ignitionrobotics.org';
/**
...
function createFuelUri(uri) {
    ...
    if (uri.indexOf(FUEL_HOST) > 0 || uri.indexOf(IGN_FUEL_HOST) > 0) {
        const uriArray = uri.split('/').filter((element) => element !== '');
...
class SpawnModel {
    constructor(scene, domElement) {
        this.active = false;
        this.plane = new THREE$1.Plane(new THREE$1.Vector3(0, 0, 1), 0);
        this.ray = new THREE$1.Ray();
        this.snapDist = undefined;
        this.scene = scene;
        this.domElement = (domElement !== undefined) ? domElement : document;
        // Material for simple shapes being spawned (grey transparent)
        this.spawnedShapeMaterial = new THREE$1.MeshPhongMaterial({ color: 0xffffff, flatShading: false });
    ...
    start(entity, callback) {
        ...
        var pos = new THREE$1.Vector2(window.window.innerWidth / 2, window.innerHeight / 2);
        var intersect = new THREE$1.Vector3();
 ...
 */
class WsLoadingManager extends LoadingManager {
    /**
     ...
     */
    filterAndEnd(url) {
        this.errorItems = this.errorItems.filter((errorUrl) => errorUrl !== url);
        this.itemEnd(url);
    }
}
...
class Scene {
    constructor(config) {
        ...
        this.COMVisual = new THREE$1.Object3D;
        this.textureCache = new Map();
        this.currentThirdPersonLookAt = new THREE$1.Vector3();
        this.defaultThirdPersonCameraOffset = new THREE$1.Vector3(-6, -2, 1.5);
        this.currentThirdPersonCameraOffset = new THREE$1.Vector3();
        this.mousePointerDown = false;
        this.currentFirstPersonLookAt = new THREE$1.Vector3();
        /**
         * Create plane
        ...
        this.createPlane = function (normal, width, height) {
            ...
            let geometry = new THREE$1.PlaneGeometry(width, height, 1, 1);
        ...
        this.emitter.on(this.moveToEntityEvent, function (entityName) {
            ...
            var bboxSize = new THREE$1.Vector3();
            var bbox = new THREE$1.Box3().setFromObject(obj);
            ...
            var endRotMat = new THREE$1.Matrix4();
            endRotMat.lookAt(endPos, targetCenter, new THREE$1.Vector3(0, 0, 1));
     ...
     */
    init() {
        THREE$1.Object3D.DefaultUp.set(0, 0, 1);
        this.name = 'default';
        ...
        // {r: 0.1, g: 0.1, b: 0.1}.
        this.ambient = new THREE$1.AmbientLight(0x191919);
        this.scene.add(this.ambient);
        // camera
        let width = this.getDomElement().width;
        let height = this.getDomElement().height;
        this.camera = new THREE$1.PerspectiveCamera(60, width / height, 0.01, 1000);
        this.resetView();
        // Clock used to time the camera 'move_to' motion.
        this.cameraMoveToClock = new THREE$1.Clock(false);
        // Start position of the camera's move_to
        this.cameraLerpStart = new THREE$1.Vector3();
        // End position of the camera's move_to
        this.cameraLerpEnd = new THREE$1.Vector3();
        // Start orientation of the camera's move_to
        this.cameraSlerpStart = new THREE$1.Quaternion();
        // End orientation of the camera's move_to
        this.cameraSlerpEnd = new THREE$1.Quaternion();
        ...
        // Grid
        this.grid = new THREE$1.GridHelper(20, 20, 0xCCCCCC, 0x4D4D4D);
        this.grid.name = 'grid';
        ...
        this.simpleShapesMaterial = new THREE$1.MeshPhongMaterial({ color: 0xffffff, flatShading: false });
        var that = this;
        ...
        this.getDomElement().addEventListener('wheel', function (event) { that.onMouseScroll(event); }, false);
        ...
        // this.timeDown = null;
        // Create a ray caster
        this.ray = new THREE$1.Raycaster();
        this.controls = new OrbitControls(this.camera, this.getDomElement());
        this.controls.mouseButtons = {
            LEFT: THREE$1.MOUSE.PAN,
            MIDDLE: THREE$1.MOUSE.ROTATE,
            RIGHT: THREE$1.MOUSE.DOLLY
        };
        // an animation loop is required with damping
        this.controls.enableDamping = false;
        this.controls.screenSpacePanning = true;
        // Bounding Box
        var indices = new Uint16Array([0, 1, 1, 2, 2, 3, 3, 0,
            4, 5, 5, 6, 6, 7, 7, 4,
            0, 4, 1, 5, 2, 6, 3, 7]);
        var positions = new Float32Array(8 * 3);
        var boxGeometry = new THREE$1.BufferGeometry();
        boxGeometry.setIndex(new THREE$1.BufferAttribute(indices, 1));
        boxGeometry.setAttribute('position', new THREE$1.BufferAttribute(positions, 3));
        this.boundingBox = new THREE$1.LineSegments(boxGeometry, new THREE$1.LineBasicMaterial({ color: 0xffffff }));
        this.boundingBox.visible = false;
        // Joint visuals
        this.jointAxis = new THREE$1.Object3D();
        this.jointAxis.name = 'JOINT_VISUAL';
        var geometry, material, mesh;
        // XYZ
        var XYZaxes = new THREE$1.Object3D();
        geometry = new THREE$1.CylinderGeometry(0.01, 0.01, 0.3, 10, 1, false);
        material = new THREE$1.MeshBasicMaterial({ color: new THREE$1.Color(0xff0000) });
        mesh = new THREE$1.Mesh(geometry, material);
        mesh.position.x = 0.15;
        mesh.rotation.z = -Math.PI / 2;
        mesh.name = 'JOINT_VISUAL';
        XYZaxes.add(mesh);
        material = new THREE$1.MeshBasicMaterial({ color: new THREE$1.Color(0x00ff00) });
        mesh = new THREE$1.Mesh(geometry, material);
        mesh.position.y = 0.15;
        mesh.name = 'JOINT_VISUAL';
        XYZaxes.add(mesh);
        material = new THREE$1.MeshBasicMaterial({ color: new THREE$1.Color(0x0000ff) });
        mesh = new THREE$1.Mesh(geometry, material);
        mesh.position.z = 0.15;
        mesh.rotation.x = Math.PI / 2;
        mesh.name = 'JOINT_VISUAL';
        XYZaxes.add(mesh);
        geometry = new THREE$1.CylinderGeometry(0, 0.03, 0.1, 10, 1, true);
        material = new THREE$1.MeshBasicMaterial({ color: new THREE$1.Color(0xff0000) });
        mesh = new THREE$1.Mesh(geometry, material);
        mesh.position.x = 0.3;
        mesh.rotation.z = -Math.PI / 2;
        ...
        var mainAxis = new THREE$1.Object3D();
        material = new THREE$1.MeshLambertMaterial();
        material.color = new THREE$1.Color(0xffff00);
        var mainAxisLen = 0.3;
        geometry = new THREE$1.CylinderGeometry(0.015, 0.015, mainAxisLen, 36, 1, false);
        mesh = new THREE$1.Mesh(geometry, material);
        mesh.position.z = mainAxisLen * 0.5;
        mesh.rotation.x = Math.PI / 2;
        mesh.name = 'JOINT_VISUAL';
        mainAxis.add(mesh);
        geometry = new THREE$1.CylinderGeometry(0, 0.035, 0.1, 36, 1, false);
        mesh = new THREE$1.Mesh(geometry, material);
        ...
        var rotAxis = new THREE$1.Object3D();
        geometry = new THREE$1.TorusGeometry(0.04, 0.006, 10, 36, Math.PI * 3 / 2);
        mesh = new THREE$1.Mesh(geometry, material);
        mesh.position.z = mainAxisLen;
        mesh.name = 'JOINT_VISUAL';
        rotAxis.add(mesh);
        geometry = new THREE$1.CylinderGeometry(0.015, 0, 0.025, 10, 1, false);
        mesh = new THREE$1.Mesh(geometry, material);
        ...
        var transAxis = new THREE$1.Object3D();
        geometry = new THREE$1.CylinderGeometry(0.01, 0.01, 0.1, 10, 1, true);
        mesh = new THREE$1.Mesh(geometry, material);
        ...
        mesh.name = 'JOINT_VISUAL';
        transAxis.add(mesh);
        geometry = new THREE$1.CylinderGeometry(0.02, 0, 0.0375, 10, 1, false);
        mesh = new THREE$1.Mesh(geometry, material);
        mesh.position.x = 0.03;
        mesh.position.y = 0.03;
        ...
        var screwAxis = new THREE$1.Object3D();
        mesh = new THREE$1.Mesh(geometry, material);
        ...
        var radius = 0.04;
        var length = 0.02;
        var curve = new THREE$1.CatmullRomCurve3([new THREE$1.Vector3(radius, 0, 0 * length),
            new THREE$1.Vector3(0, radius, 1 * length),
            new THREE$1.Vector3(-radius, 0, 2 * length),
            new THREE$1.Vector3(0, -radius, 3 * length),
            new THREE$1.Vector3(radius, 0, 4 * length),
            new THREE$1.Vector3(0, radius, 5 * length),
            new THREE$1.Vector3(-radius, 0, 6 * length)]);
        geometry = new THREE$1.TubeGeometry(curve, 36, 0.01, 10, false);
        mesh = new THREE$1.Mesh(geometry, material);
        mesh.position.z = mainAxisLen - 0.23;
        mesh.name = 'JOINT_VISUAL';
        screwAxis.add(mesh);
        this.jointAxis['screwAxis'] = screwAxis;
        var ballVisual = new THREE$1.Object3D();
        geometry = new THREE$1.SphereGeometry(0.06);
        mesh = new THREE$1.Mesh(geometry, material);
        mesh.name = 'JOINT_VISUAL';
        ballVisual.add(mesh);
        ...
        geometry = new THREE$1.SphereGeometry(1, 32, 32);
    ...
    addSky(cubemap) {
        if (cubemap === undefined) {
            const cubeLoader = new THREE$1.CubeTextureLoader();
            ...
            (compressedTexture) => {
                ...
                this.scene.background = new THREE$1.CubeTexture(reorderImages, compressedTexture.mapping, compressedTexture.wrapS, compressedTexture.wrapT, compressedTexture.magFilter, compressedTexture.minFilter, compressedTexture.format, compressedTexture.type, compressedTexture.anisotropy, compressedTexture.encoding);
                ...
                if (this.findResourceCb) {
                    ...
                    this.findResourceCb(cubemap, (material, error) => {
                        ...
                        const reorderImages = [images[1], images[0],
                            images[2], images[3],
                            images[5], images[4]];
                        this.scene.background = new THREE$1.CubeTexture(reorderImages);
                        this.scene.background.format =
                            (texDatas['format']);
                        if (texDatas['mipmapCount'] === 1) {
                            this.scene.background.minFilter = THREE$1.LinearFilter;
                        }
                        this.scene.background.needsUpdate = true;
                        // Mark the texture as done in the loading manager.
                        const manager = this.ddsLoader.manager;
    ...
    addFog(color, density, changeBackground) {
        this.scene.fog = new THREE$1.FogExp2(color, density);
        if (changeBackground === true) {
            this.scene.background = new THREE$1.Color(color);
        }
    }
    initScene() {
    ...
    onPointerDown(event) {
        ...
        {*/
        pos = new THREE$1.Vector2(event.clientX, event.clientY);
        if (event.which !== 1) {
            mainPointer = false;
        }
        //}
    ...
    onMouseScroll(event) {
        event.preventDefault();
        const pos = new THREE$1.Vector2(event.clientX, event.clientY);
        let intersect = new THREE$1.Vector3();
    ...
    getRayCastModel(pos, intersect) {
        ...
        var vector = new THREE$1.Vector2(((pos.x - rect.x) / rect.width) * 2 - 1, -((pos.y - rect.y) / rect.height) * 2 + 1);
        ...
        let model = new THREE$1.Object3D();
        var point;
        if (objects.length > 0) {
    ...
    render(timeElapsedMs) {
        ...
        if (this.cameraMode === this.followEntityEvent) {
            ...
            let fixedLookAt = new THREE$1.Vector3(12, -4, 0);
            ...
            // +Z - Towards the camera
            // -Z - Away from the camera
            let fixedCameraOffset = new THREE$1.Vector3(-0.12, 0, 0.6);
            fixedCameraOffset.applyQuaternion(this.cameraTrackObject.quaternion);
            fixedCameraOffset.add(this.cameraTrackObject.position);
            let fixedLookAt = new THREE$1.Vector3(6, 0, 0);
            fixedLookAt.applyQuaternion(this.cameraTrackObject.quaternion);
            fixedLookAt.add(this.cameraTrackObject.position);
            ...
            if (lerp >= 1.0) {
                ...
                THREE$1.Quaternion.slerp(this.cameraSlerpStart, this.cameraSlerpEnd, this.camera.quaternion, lerp);
            }
        }
        // this.modelManipulator.update();
        /*if (this.radialMenu)
        ...
        if (this.controls) {
            ...
            if (!this.shadowTmpV)
                this.shadowTmpV = new THREE$1.Vector3();
            if (!this.shadowTargetAnchor) {
                this.shadowTargetAnchor = new THREE$1.Vector3(shadowTarget.x, shadowTarget.y, shadowTarget.z);
            }
            ...
            this.scene.traverse(function (child) {
                ...
                if (directionalLight.isDirectionalLight &&
                    ...
                    var len = Math.sqrt(shadowDirection.x * shadowDirection.x +
                        shadowDirection.y * shadowDirection.y +
                        shadowDirection.z * shadowDirection.z) || 1;
     ...
     */
    createSphere(radius) {
        var geometry = new THREE$1.SphereGeometry(radius, 32, 32);
        var mesh = new THREE$1.Mesh(geometry, this.simpleShapesMaterial);
        return mesh;
    }
    /**
     * Create cylinder
    ...
    createCylinder(radius, length) {
        var geometry = new THREE$1.CylinderGeometry(radius, radius, length, 32, 1, false);
        var mesh = new THREE$1.Mesh(geometry, this.simpleShapesMaterial);
    ...
    createBox(width, height, depth) {
        let geometry = new THREE$1.BoxGeometry(width, height, depth, 1, 1, 1);
    ...
    createPointLight(obj, color, intensity, distance, cast_shadows) {
        ...
        var lightObj = new THREE$1.PointLight(color, intensity);
        if (distance) {
    ...
    createSpotLight(obj, color, intensity, distance, cast_shadows, inner_angle, outer_angle, falloff, direction) {
        ...
        let lightObj = new THREE$1.SpotLight(color, intensity, distance);
    ...
    loadHeightmap(heights, width, height, segmentWidth, segmentHeight, origin, textures, blends, parent) {
        ...
        let geometry = new THREE$1.PlaneGeometry(width, height, (segmentWidth - 1) * scale, (segmentHeight - 1) * scale);
        ...
        if (textures && textures.length > 0) {
            ...
            for (let t = 0; t < textures.length; ++t) {
                ...
                configTexture(texturesLoaded[t], new THREE$1.Vector2(width / textures[t].size, height / textures[t].size));
                let normalUri;
                if (textures[t].normal) {
                    normalUri = createFuelUri(textures[t].normal);
                    normalsLoaded[t] = this.loadTexture(normalUri);
                    configTexture(normalsLoaded[t], new THREE$1.Vector2(width / textures[t].size, height / textures[t].size));
                }
            }
            ...
            material = new THREE$1.MeshStandardMaterial(materialOptions);
        }
        else {
            material = new THREE$1.MeshPhongMaterial({ color: 0x555555 });
        }
    ...
    prepareColladaMesh(dae) {
        ...
        for (let i = 0; i < allChildren.length; ++i) {
            if (allChildren[1] && allChildren[i] instanceof THREE$1.Light &&
    ...
    useSubMesh(mesh, submesh, centerSubmesh) {
        ...
        function lookForSubmesh(obj, parent) {
            if (obj instanceof THREE$1.Mesh) {
                // Check if the mesh has the correct name and has geometry.
                if (obj.name === submesh && obj.hasOwnProperty('geometry')) {
                    // Center the submesh.
                    if (centerSubmesh) {
                        // obj file
                        if (obj.geometry instanceof THREE$1.BufferGeometry) {
                            let geomPosition = obj.geometry.getAttribute('position');
                            let minPos = new THREE$1.Vector3();
                            let maxPos = new THREE$1.Vector3();
                            let centerPos = new THREE$1.Vector3();
                            ...
                            for (let i = 0; i < geomPosition.count; i++) {
                                let origPos = new THREE$1.Vector3();
                ...
                for (let i = 0; i < obj.children.length; i++) {
                    if (obj.children[i] instanceof THREE$1.Mesh ||
                        obj.children[i] instanceof THREE$1.Group) {
        ...
        for (var i = 0; i < mesh.children.length; i++) {
            if (mesh.children[i] instanceof THREE$1.Mesh ||
                mesh.children[i] instanceof THREE$1.Group) {
    ...
    setMaterial(obj, material) {
        if (obj) {
            if (material) {
                ...
                if (material.pbr) {
                    obj.material = new THREE$1.MeshStandardMaterial();
                    ...
                    if (material.pbr.albedoMap) {
                        ...
                        if (albedoMap.format === THREE$1.RGBAFormat) {
                    ...
                    maps.forEach(function (map) {
                        map.wrapS = map.wrapT = THREE$1.RepeatWrapping;
                        map.repeat.x = 1.0;
                        map.repeat.y = 1.0;
                        if (material.scale) {
                    ...
                    obj.material = new THREE$1.MeshPhongMaterial();
                    ...
                    if (material.texture) {
                        ...
                        // enable alpha test for textures with alpha transparency
                        if (texture.format === THREE$1.RGBAFormat) {
                            obj.material.alphaTest = 0.5;
                        }
                    }
    ...
    createThumbnails(filename, center) {
        ...
        const lightTarget = new THREE$1.Object3D();
        lightTarget.name = 'thumbnails_light_target';
        lightTarget.position.copy(center);
        this.scene.add(lightTarget);
        const light = new THREE$1.DirectionalLight(0xffffff, 1.0);
        ...
        this.camera.position.add(new THREE$1.Vector3(1.6, -1.6, 1.2));
        ...
        // Top
        this.camera.position.copy(center);
        this.camera.position.add(new THREE$1.Vector3(0, 0, 2.2));
        this.camera.rotation.copy(new THREE$1.Euler(0, 0, -90 * Math.PI / 180));
        light.position.copy(this.camera.position);
        this.render(0);
        ...
        // Front
        this.camera.position.copy(center);
        this.camera.position.add(new THREE$1.Vector3(2.2, 0, 0));
        this.camera.rotation.copy(new THREE$1.Euler(0, 90 * Math.PI / 180, 90 * Math.PI / 180));
        light.position.copy(this.camera.position);
        this.render(0);
        ...
        // Side
        this.camera.position.copy(center);
        this.camera.position.add(new THREE$1.Vector3(0, 2.2, 0));
        this.camera.rotation.copy(new THREE$1.Euler(-90 * Math.PI / 180, 0, 180 * Math.PI / 180));
        light.position.copy(this.camera.position);
        this.render(0);
        ...
        this.camera.position.copy(center);
        this.camera.position.add(new THREE$1.Vector3(-2.2, 0, 0));
        this.camera.rotation.copy(new THREE$1.Euler(90 * Math.PI / 180, -90 * Math.PI / 180, 0));
        light.position.copy(this.camera.position);
        light.position.add(new THREE$1.Vector3(-2000, 0, 0));
        this.render(0);
    ...
    onRightClick(event, callback) {
        var pos = new THREE$1.Vector2(event.clientX, event.clientY);
        var model = this.getRayCastModel(pos, new THREE$1.Vector3());
        if (model && model.name !== '' && model.name !== 'plane' /* &&
            this.modelManipulator.pickerNames.indexOf(model.name) === -1*/) {
            callback(model);
        }
        ...
        // Create visuals
            ...
            for (var j = 0; j < model.joint.length; ++j) {
                ...
                if (mainAxis) {
                    ...
                    direction = new THREE$1.Vector3(model.joint[j].axis1.xyz.x, model.joint[j].axis1.xyz.y, model.joint[j].axis1.xyz.z);
                    direction.normalize();
                    tempMatrix = new THREE$1.Matrix4();
                    ...
                    rotMatrix = new THREE$1.Matrix4();
                    rotMatrix.lookAt(direction, new THREE$1.Vector3(0, 0, 0), mainAxis.up);
                ...
                if (secondAxis) {
                    ...
                    direction = new THREE$1.Vector3(model.joint[j].axis2.xyz.x, model.joint[j].axis2.xyz.y, model.joint[j].axis2.xyz.z);
                    direction.normalize();
                    tempMatrix = new THREE$1.Matrix4();
                    ...
                    rotMatrix = new THREE$1.Matrix4();
                    rotMatrix.lookAt(direction, new THREE$1.Vector3(0, 0, 0), secondAxis.up);
     ...
     */
    addModelLighting() {
        this.ambient.color = new THREE$1.Color(0x666666);
        ...
        0.5, 
        // Pose
        new Pose(new THREE$1.Vector3(0, 10, 10), new THREE$1.Quaternion(0, 0, 0, 1)), 
        // Distance
        undefined, 
        // Cast shadows
        true, 
        ...
        '__model_light1__', 
        // Direction
        new THREE$1.Vector3(0, -0.707, -0.707), 
        // Specular
        ...
        0.4, 
        // Pose
        new Pose(new THREE$1.Vector3(-10, -10, 10), new THREE$1.Quaternion(0, 0, 0, 1)), 
        // Distance
        undefined, 
        // Cast shadows
        true, 
        ...
        '__model_light2__', 
        // Direction
        new THREE$1.Vector3(0.707, 0.707, 0), 
        // Specular
        new Color(0.3, 0.3, 0.3, 1.0));
        this.add(light3);
            ...
            if (obj.material) {
                ...
                obj.material.forEach(function (material) {
                    Object.keys(material).forEach(function (property) {
                        if (material[property] instanceof THREE$1.Texture) {
    ...
    setRequestHeader(header, value) {
        ...
        if (this.requestHeader) {
            this.textureLoader.load = function (url, onLoad, onProgress, onError) {
                var fileLoader = new THREE$1.FileLoader();
                fileLoader.setResponseType('blob');
                fileLoader.setRequestHeader(this.requestHeader);
                let texture = new THREE$1.Texture();
        ...
        let fallbackLoader = (map, texture) => {
            if (this.findResourceCb) {
                ...
                this.findResourceCb(map, (image, error) => {
                    ...
                    imageElem.src += window.btoa(binary);
                    texture.format = isJPEG ? THREE$1.RGBFormat : THREE$1.RGBAFormat;
                    texture.needsUpdate = true;
...
class SDFParser {
    ...
    **/
    ...
    parse3DVector(vectorInput) {
        let vector3D = new THREE$1.Vector3();
        var values = [];
        ...
        return new THREE$1.Vector3(vector3D.x, vector3D.y, vector3D.z);
    }
    /**
    ...
    spawnLightFromSDF(sdfObj) {
        ...
        let lightObj = this.scene.createLight(type, diffuse, intensity, pose, distance, castShadows, name, direction, specular, attConst, attLin, attQuad, innerAngle, outerAngle, falloff);
        return lightObj;
    }
    /**
    ...
    spawnLightFromProto(light) {
        ...
        let lightObj = this.scene.createLight(
    ...
    parsePose(poseInput) {
        ...
        // get euler rotation and convert it to Quaternion
        var euler = new THREE$1.Euler(parseFloat(values[3]), parseFloat(values[4]), parseFloat(values[5]), 'ZYX');
    ...
    parseScale(scaleInput) {
        ...
        var scale = new THREE$1.Vector3(parseFloat(values[0]), parseFloat(values[1]), parseFloat(values[2]));
        return scale;
    }
    /**
    ...
    createMaterial(srcMaterial) {
        ...
        if (srcMaterial.normal_map) {
            ...
            if (mapUri != '') {
                ...
                var normalMapName = srcMaterial.normal_map.substr(startIndex, srcMaterial.normal_map.lastIndexOf('.') - startIndex);
        ...
        if (material.pbr && this.enablePBR) {
            ...
            if (material.pbr.albedoMap) {
                let albedoMap = '';
                let albedoMapName = material.pbr.albedoMap.split('/').pop();
            ...
            if (material.pbr.emissiveMap) {
                let emissiveMap = '';
                let emissiveMapName = material.pbr.emissiveMap.split('/').pop();
            ...
            if (material.pbr.normalMap) {
                let pbrNormalMap = '';
                let pbrNormalMapName = material.pbr.normalMap.split('/').pop();
            ...
            if (material.pbr.roughnessMap) {
                let roughnessMap = '';
                let roughnessMapName = material.pbr.roughnessMap.split('/').pop();
            ...
            if (material.pbr.metalnessMap) {
                let metalnessMap = '';
                let metalnessMapName = material.pbr.metalnessMap.split('/').pop();
    ...
    parseSize(sizeInput) {
        if (typeof sizeInput === 'string') {
            let values = sizeInput.split(/\s+/);
            return new THREE$1.Vector3(parseFloat(values[0]), parseFloat(values[1]), parseFloat(values[2]));
        }
        return new THREE$1.Vector3(sizeInput.x, sizeInput.y, sizeInput.z);
    }
    /**
    ...
    createGeom(geom, mat, parent, options) {
        ...
        let obj = undefined;
        let size;
        let normal = new THREE$1.Vector3(0, 0, 1);
        ...
        if (geom.box) {
            ...
            let meshUri = geom.mesh.uri || geom.mesh.filename;
            let submesh = '';
            let centerSubmesh = false;
            let modelName = '';
            ...
            let modelUri = createFuelUri(modelName);
            let ext = modelUri.substr(-4).toLowerCase();
            let materialName = parent.name + '::' + modelUri;
            this.entityMaterial[materialName] = material;
            let meshFileName = meshUri.substring(meshUri.lastIndexOf('/'));
            if (!this.usingFilesUrls) {
            ...
            this.scene.loadHeightmap(geom.heightmap.heights, geom.heightmap.size.x, geom.heightmap.size.y, geom.heightmap.width, geom.heightmap.height, new THREE$1.Vector3(geom.heightmap.origin.x, geom.heightmap.origin.y, geom.heightmap.origin.z), geom.heightmap.texture, geom.heightmap.blend, parent);
        }
        if (obj) {
            if (material) {
                ...
                if (ext !== '.stl') {
                    ...
                    for (let c = 0; c < allChildren.length; ++c) {
                        if (allChildren[c] instanceof THREE$1.Mesh) {
                            ...
                            let isColladaWithTexture = ext === '.dae' &&
                                !!allChildren[c].material &&
                                !!allChildren[c].material.map;
        ...
        function loadGeom(visualObj) {
            ...
            for (var r = 0; r < toRemove.length; ++r) {
                try {
                    if (toRemove[r].parent)
                        toRemove[r].parent.remove(toRemove[r]);
                    if (toRemove[r].geometry)
                        toRemove[r].geometry.dispose();
                    var mats = toRemove[r].material;
                    if (mats) {
                        const materials = Array.isArray(mats) ? mats : [mats];
                        for (var m = 0; m < materials.length; ++m) {
                            const textureMap = materials[m].map;
                            if (textureMap)
                                textureMap.dispose();
                            materials[m].dispose();
                        }
    ...
    createVisual(visual, options) {
        let visualObj = new THREE$1.Object3D();
    ...
    createSensor(sensor, options) {
        let sensorObj = new THREE$1.Object3D();
        sensorObj.name = sensor['name'] || sensor['@name'] || '';
        if (sensor.pose) {
            let sensorPose = this.parsePose(sensor.pose);
    ...
    loadSDF(sdfName, callback) {
        if (!sdfName) {
            let m = 'Must provide either a model/world name or the URL of an SDF file';
            console.error(m);
            return;
        }
        let lowerCaseName = sdfName.toLowerCase();
        let filename = '';
    ...
    spawnModelFromSDF(sdfObj, options) {
        // create the model
        let modelObj = new THREE$1.Object3D();
    ...
    spawnWorldFromSDF(sdfObj, options) {
        var worldObj = new THREE$1.Object3D();
    ...
    includeModel(includedModel, parent) {
        ...
        const model = Object.assign(Object.assign({}, includedModel), { parent: parent });
    ...
    createLink(link, options) {
        ...
        let visualObj;
        let sensorObj;
        let linkObj = new THREE$1.Object3D();
        linkObj.name = link['name'] || link['@name'] || '';
        if (link.inertial) {
            let inertialPose;
            let inertialMass;
            let inertia = new Inertia();
     ...
     */
    createParticleEmitter(emitter, parent) {
        ...
        const particleEmitterObj = new THREE$1.Object3D();
        // Given name of the emitter.
        this.createUniqueName(emitter);
        ...
        // Emitter type.
        extractValue('type') || extractValue('@type') || 'point';
        ...
        const size = this.parse3DVector(extractedSize) || new THREE$1.Vector3(1, 1, 1);
        // Size of the individual particles.
        const extractedParticleSize = extractValue('particle_size');
        const particleSize = this.parse3DVector(extractedParticleSize) || new THREE$1.Vector3(1, 1, 1);
        // Pose of the particle emitter
        const extractedPose = extractValue('pose');
        this.parsePose(extractedPose);
        ...
        if (!nebulaSystem) {
            ...
            nebulaRenderer = new SpriteRenderer(this.scene.scene, THREE$1);
        ...
        const createSprite = () => {
            ...
            const material = new THREE$1.SpriteMaterial({
                map,
                transparent: true,
            });
            return new THREE$1.Sprite(material);
        };
        ...
        const particleRate = new Rate(1, 1 / rate);
        const particleSizeInitializer = new Radius(particleSize.x, particleSize.y);
        const particleVelocityInitializer = new VectorVelocity(new THREE$1.Vector3(1, 0, 0), 0);
     ...
     */
    addModelByType(model, type) {
        let sdf;
        let translation = new THREE$1.Vector3();
        let quaternion = new THREE$1.Quaternion();
        let modelObj;
        let that = this;
        if (model.matrixWorld) {
            let matrix = model.matrixWorld;
            let scale = new THREE$1.Vector3();
            matrix.decompose(translation, quaternion, scale);
        }
        let euler = new THREE$1.Euler();
    ...
    createUniqueName(obj) {
        let objectName = obj['name'] || obj['@name'] || '';
        let objectId = obj['id'] || obj['@id'] || '';
...
class SceneManager {
    ...
    getConnectionStatusAsObservable() {
        return this.transport.getConnectionStatus().pipe(map((status) => status === 'ready'));
    }
    /**
    ...
    connect(url, key) {
        ...
        this.sceneInfoSubscription = this.transport.sceneInfo$.subscribe((sceneInfo) => {
            ...
            if (sceneInfo['ambient'] !== undefined &&
                ...
                this.scene.ambient.color = new THREE$1.Color(sceneInfo['ambient']['r'], sceneInfo['ambient']['g'], sceneInfo['ambient']['b']);
            }
        });
    }
    /**
...
export { Asset, AssetViewer, AudioTopic, Color, FuelServer, Gamepad, Inertia, Material, ModelUserData, PBRMaterial, Pose, Publisher, SDFParser, Scene, SceneManager, Topic, Transport, binaryToBase64, binaryToImage, getDescendants };
