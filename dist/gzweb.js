(function (global, factory) {
...
})(this, (function (exports, THREE$1, eventemitter2, JSZip, fastXmlParser, System, rxjs, protobufjs) { 'use strict';
    ...
    _createClass(TGALoader, [{
      ...
      value: function parse(buffer) {
        ...
        return {
          ...
          minFilter: THREE$1.LinearMipmapLinearFilter
        };
      }
    }]);

    return TGALoader;
  }(THREE$1.DataTextureLoader);
  ...
  var ColladaLoader = /*#__PURE__*/function (_Loader) {
    ...
    _createClass(ColladaLoader, [{
      ...
      value: function load(url, onLoad, onProgress, onError) {
        var scope = this;
        var path = scope.path === '' ? THREE$1.LoaderUtils.extractUrlBase(url) : scope.path;
      ...
      value: function parse(text, path) {
        ...
        function parseAnimation(xml) {
          ...
          if (hasChildren === false) {
            ...
            library.animations[xml.getAttribute('id') || THREE$1.MathUtils.generateUUID()] = data;
          }
        }
        ...
        function buildSkin(data) {
          ...
          if (data.bindShapeMatrix) {
            ...
            build.bindMatrix = new THREE$1.Matrix4().identity();
        ...
        function buildMaterial(data) {
            ...
            if (image !== null) {
              ...
              if (loader !== undefined) {
                  ...
                  function (error) {
                    if (scope.findResourceCb) {
                      ...
                      scope.findResourceCb(filename, function (imageBytes, error) {
                        ...
                        imageElem.src = isJPEG ? "data:image/jpg;base64," : "data:image/png;base64,";
                        imageElem.src += window.btoa(binary);
                        scopeTexture.format = isJPEG ? THREE$1.RGBFormat : THREE$1.RGBAFormat;
                        scopeTexture.needsUpdate = true;
                ...
                if (extra !== undefined && extra.technique !== undefined && isEmpty(extra.technique) === false) {
                  ...
                  texture.wrapS = _technique.wrapU ? THREE$1.RepeatWrapping : THREE$1.ClampToEdgeWrapping;
                  texture.wrapT = _technique.wrapV ? THREE$1.RepeatWrapping : THREE$1.ClampToEdgeWrapping;
          ...
          for (var key in parameters) {
            ...
            switch (key) {
              case 'diffuse':
                ...
                if (parameter.texture) material.map = getTexture(parameter.texture, THREE$1.LinearEncoding);
                break;
              ...
              case 'ambient':
                if (parameter.texture) material.lightMap = getTexture(parameter.texture, THREE$1.LinearEncoding);
                break;
              ...
              case 'emission':
                ...
                if (parameter.texture) material.emissiveMap = getTexture(parameter.texture, THREE$1.LinearEncoding);
                break;
            }
          } // Modified by German Mas.
          ...
          if (technique.extra !== undefined && technique.extra.technique !== undefined) {
            ...
            for (var k in techniques) {
              ...
              switch (k) {
                case 'double_sided':
                  material.side = v === 1 ? THREE$1.DoubleSide : THREE$1.FrontSide;
                  break;

                case 'bump':
                  material.normalMap = getTexture(v.texture);
                  material.normalScale = new THREE$1.Vector2(1, 1);
                  break;
              }
            }
          }
        ...
        function buildCamera(data) {
          ...
          switch (data.optics.technique) {
            case 'perspective':
              camera = new THREE$1.PerspectiveCamera(data.optics.parameters.yfov, data.optics.parameters.aspect_ratio, data.optics.parameters.znear, data.optics.parameters.zfar);
              break;
            ...
            case 'orthographic':
              ...
              xmag *= 0.5;
              ymag *= 0.5;
              camera = new THREE$1.OrthographicCamera(-xmag, xmag, ymag, -ymag, // left, right, top, bottom
          ...
          for (var i = 0, l = xml.childNodes.length; i < l; i++) {
            ...
            switch (child.nodeName) {
              case 'color':
                ...
                data.color = new THREE$1.Color().fromArray(array).convertSRGBToLinear();
                break;
        ...
        function buildGeometryType(primitives, sources, vertices) {
          ...
          if (uv2.array.length > 0) geometry.setAttribute('uv2', new THREE$1.Float32BufferAttribute(uv2.array, uv2.stride));
        ...
        function parseKinematicsTransform(xml) {
          ...
          switch (data.type) {
            ...
            case 'rotate':
              data.obj = new THREE$1.Vector3();
              data.obj.fromArray(array);
              data.angle = THREE$1.MathUtils.degToRad(array[3]);
              break;
          }
                ...
                if (value > joint.limits.max || value < joint.limits.min) {
                  ...
                  for (var _i2 = 0; _i2 < transforms.length; _i2++) {
                    ...
                    if (transform.sid && transform.sid.indexOf(jointIndex) !== -1) {
                      switch (joint.type) {
                        case 'revolute':
                          matrix.multiply(m0.makeRotationAxis(_axis, THREE$1.MathUtils.degToRad(value)));
                          break;
          ...
          for (var i = 0; i < xml.childNodes.length; i++) {
            ...
            switch (child.nodeName) {
              case 'matrix':
                ...
                var _matrix = new THREE$1.Matrix4().fromArray(array).transpose();
              ...
              case 'scale':
                array = parseFloats(child.textContent);
                _vector = new THREE$1.Vector3().fromArray(array);
                transforms.push({
                  sid: child.getAttribute('sid'),
                  type: child.nodeName,
                  obj: _vector
              ...
              case 'rotate':
                array = parseFloats(child.textContent);
                _vector = new THREE$1.Vector3().fromArray(array);
                var angle = THREE$1.MathUtils.degToRad(array[3]);
                transforms.push({
                  sid: child.getAttribute('sid'),
                  type: child.nodeName,
                  obj: _vector,
        ...
        var fallbackMaterial = new THREE$1.MeshBasicMaterial({
          color: 0xff00ff
        });
        ...
        var tempColor = new THREE$1.Color();
        var animations = [];
        var kinematics = {};
        var count = 0; //
  ...
  var DDSLoader = /*#__PURE__*/function (_CompressedTextureLoa) {
    ...
    _createClass(DDSLoader, [{
      ...
      value: function parse(buffer, loadMipmaps) {
        ...
        switch (fourCC) {
          case FOURCC_DXT1:
            blockBytes = 8;
            dds.format = THREE$1.RGB_S3TC_DXT1_Format;
            break;

          case FOURCC_DXT3:
            blockBytes = 16;
            dds.format = THREE$1.RGBA_S3TC_DXT3_Format;
            break;

          case FOURCC_DXT5:
            blockBytes = 16;
            dds.format = THREE$1.RGBA_S3TC_DXT5_Format;
            break;

          case FOURCC_ETC1:
            blockBytes = 8;
            dds.format = THREE$1.RGB_ETC1_Format;
            break;
    ...
    return DDSLoader;
  }(THREE$1.CompressedTextureLoader);
  ...
  var _vA = new THREE$1.Vector3();

  var _vB = new THREE$1.Vector3();

  var _vC = new THREE$1.Vector3();

  var _ab = new THREE$1.Vector3();

  var _cb = new THREE$1.Vector3();

  var _color = new THREE$1.Color();
      ...
      value: function parse(text) {
        ...
        if (hasPrimitives === true) {
          for (var _i = 0, _l2 = state.objects.length; _i < _l2; _i++) {
            ...
            for (var mi = 0, miLen = materials.length; mi < miLen; mi++) {
              ...
              if (this.materials !== null) {
                ...
                if (isLine && _material && !(_material instanceof THREE$1.LineBasicMaterial)) {
                  var materialLine = new THREE$1.LineBasicMaterial();
                  THREE$1.Material.prototype.copy.call(materialLine, _material);
                  materialLine.color.copy(_material.color);
                  _material = materialLine;
                } else if (isPoints && _material && !(_material instanceof THREE$1.PointsMaterial)) {
                  var materialPoints = new THREE$1.PointsMaterial({
                    size: 10,
                    sizeAttenuation: false
                  });
                  THREE$1.Material.prototype.copy.call(materialPoints, _material);
              ...
              if (_material === undefined) {
                if (isLine) {
                  _material = new THREE$1.LineBasicMaterial();
                } else if (isPoints) {
                  _material = new THREE$1.PointsMaterial({
                    size: 1,
                    sizeAttenuation: false
                  });
                } else {
                  _material = new THREE$1.MeshPhongMaterial();
                }
          ...
          if (state.vertices.length > 0) {
            var _material2 = new THREE$1.PointsMaterial({
              size: 1,
              sizeAttenuation: false
            });

            var _buffergeometry = new THREE$1.BufferGeometry();

            _buffergeometry.setAttribute('position', new THREE$1.Float32BufferAttribute(state.vertices, 3));

            if (state.colors.length > 0 && state.colors[0] !== undefined) {
              _buffergeometry.setAttribute('color', new THREE$1.Float32BufferAttribute(state.colors, 3));

              _material2.vertexColors = true;
            }

            var points = new THREE$1.Points(_buffergeometry, _material2);
  ...
  var MTLLoader = /*#__PURE__*/function (_Loader) {
    ...
    _createClass(MTLLoader, [{
      ...
      value: function load(url, onLoad, onProgress, onError) {
        var scope = this;
        var path = this.path === '' ? THREE$1.LoaderUtils.extractUrlBase(url) : this.path;
        var loader = new THREE$1.FileLoader(this.manager);
        loader.setPath(this.path);
        loader.setRequestHeader(this.requestHeader);
        loader.setWithCredentials(this.withCredentials);
  ...
  var MaterialCreator = /*#__PURE__*/function () {
    function MaterialCreator() {
      ...
      this.side = this.options.side !== undefined ? this.options.side : THREE$1.FrontSide;
      this.wrap = this.options.wrap !== undefined ? this.options.wrap : THREE$1.RepeatWrapping;
    }
      ...
      value: function createMaterial_(materialName) {
        ...
        for (var prop in mat) {
          ...
          switch (prop.toLowerCase()) {
            ...
            case 'kd':
              // Diffuse color (color under white light) using RGB values
              params.color = new THREE$1.Color().fromArray(value).convertSRGBToLinear();
              break;

            case 'ks':
              // Specular color (color when light is reflected from shiny surface) using RGB values
              params.specular = new THREE$1.Color().fromArray(value).convertSRGBToLinear();
              break;

            case 'ke':
              // Emissive using RGB values
              params.emissive = new THREE$1.Color().fromArray(value).convertSRGBToLinear();
              break;
      ...
      value: function loadMTL(_text) {
        ...
        for (var i in lines) {
          ...
          if ((line.indexOf('/materials/textures') > 0 || line.indexOf('../materials/') > 0) && !this.usingRawFiles) {
            if (this.mtlLoader.path && this.mtlLoader.path.indexOf('http') === 0 && line.indexOf('../materials/') > 0) {
          ...
          if ((line.indexOf('../materials/textures') > 0 || line.indexOf('../materials/') > 0) && this.usingRawFiles) {
            if (this.uri && this.uri.indexOf('http') === 0) {
              ...
              var modelDir = meshDir.indexOf('/meshes/') > -1 ? meshDir.substr(0, meshDir.lastIndexOf('/meshes/') + 1) : meshDir;
  ...
  var OrbitControls = /*#__PURE__*/function (_EventDispatcher) {
    ...
    function OrbitControls(object, domElement) {
      ...
      _this.target = new THREE$1.Vector3(); // How far you can dolly in and out ( PerspectiveCamera only )
        ...
        return function update() {
          ...
          if (Math.abs(sphericalDelta.phi - 0) > 0.001 || Math.abs(sphericalDelta.theta - 0) > 0.001) {
            var rotateAroundWorldAxis = function rotateAroundWorldAxis(object, axis, radians) {
              var rotWorldMatrix = new THREE$1.Matrix4();
            ...
            rotateAroundWorldAxis(scope.object, new THREE$1.Vector3(0, 0, 1), spherical.theta - oldTheta);
            var localPitch = new THREE$1.Vector3(1, 0, 0);
  ...
  }(THREE$1.EventDispatcher); // This set of controls performs orbiting, dollying (zooming), and panning.
  ...
  var Pose = /*#__PURE__*/_createClass(function Pose(pos, rot) {
    ...
    this.orientation = new THREE$1.Quaternion();
    ...
    _createClass(STLLoader, [{
      ...
      value: function parse(data) {
        ...
        function parseBinary(data) {
          ...
          geometry.setAttribute('normal', new THREE$1.BufferAttribute(normals, 3));

          if (hasColors) {
            geometry.setAttribute('color', new THREE$1.BufferAttribute(colors, 3));
    ...
    _createClass(Scene, [{
      ...
      value: function init() {
        ...
        this.controls.mouseButtons = {
          ...
          MIDDLE: THREE__namespace.MOUSE.ROTATE,
          RIGHT: THREE__namespace.MOUSE.DOLLY
      ...
      value: function render(timeElapsedMs) {
        ...
        if (this.controls) {
          ...
          if (!this.shadowTmpV) this.shadowTmpV = new THREE__namespace.Vector3();

          if (!this.shadowTargetAnchor) {
            this.shadowTargetAnchor = new THREE__namespace.Vector3(shadowTarget.x, shadowTarget.y, shadowTarget.z);
          }
          ...
          this.scene.traverse(function (child) {
            var directionalLight = child;
            var shadowDirection = directionalLight.userData.shadowDirection;

            if (directionalLight.isDirectionalLight && directionalLight.castShadow && shadowDirection) {
              var len = Math.sqrt(shadowDirection.x * shadowDirection.x + shadowDirection.y * shadowDirection.y + shadowDirection.z * shadowDirection.z) || 1;
      ...
      value: function createSpotLight(obj, color, intensity, distance, cast_shadows, inner_angle, outer_angle, falloff, direction) {
        ...
        lightObj.castShadow = cast_shadows !== undefined ? !!cast_shadows : true; // Set the target
      ...
      value: function loadSTL(uri, submesh, centerSubmesh, onLoad, onError) {
        ...
        function (error) {
          if (that.findResourceCb) {
            // Get the mesh from the websocket server.
            that.findResourceCb(uri, function (mesh, error) {
              if (error !== undefined) {
                // Mark the mesh as error in the loading manager.
      ...
      value: function createThumbnails(filename, center) {
        ...
        Promise.all(promises).then(function () {
          ...
          _this4.scene.remove(light);

          _this4.scene.remove(lightTarget);
        });
      }
      /**
       * Show radial menu
       ...
       */
      function onRightClick(event, callback) {
        var pos = new THREE__namespace.Vector2(event.clientX, event.clientY);
        ...
        var fallbackLoader = function fallbackLoader(map, texture) {
          if (_this5.findResourceCb) {
            ...
            _this5.findResourceCb(map, function (image, error) {
              ...
              var manager = _this5.textureLoader.manager;
    ...
    _createClass(SDFParser, [{
      ...
      value: function createGeom(geom, mat, parent, options) {
        ...
        function loadGeom(visualObj) {
          ...
          for (var c = 0; c < allChildren.length; ++c) {
            if (allChildren[c] instanceof THREE__namespace.Mesh) {
              var _mesh = allChildren[c];
              var mName = (_mesh.name || '').toLowerCase();
              ...
              if (isGround && _mesh.material) {
                if (Array.isArray(_mesh.material)) {
                  _mesh.material.forEach(function (material) {
                    material.polygonOffset = true;
                    material.polygonOffsetFactor = 4;
                    material.polygonOffsetUnits = 4;
                  });
                } else {
                  _mesh.material.polygonOffset = true;
                  _mesh.material.polygonOffsetFactor = 4;
                  _mesh.material.polygonOffsetUnits = 4;
                }

                _mesh.renderOrder = -1;
              }

              _mesh.castShadow = !(isGround || isEnclosure);
              _mesh.receiveShadow = true;

              if (visualObj.castShadow) {
                _mesh.castShadow = visualObj.castShadow;
              }

              if (visualObj.receiveShadow) {
                _mesh.receiveShadow = visualObj.receiveShadow;
              }

              if (visualObj.name.indexOf('COLLISION_VISUAL') >= 0) {
                _mesh.castShadow = false;
                _mesh.receiveShadow = false;
                _mesh.visible = that.scene.showCollisions;
              }
            }
          }
          ...
          for (var r = 0; r < toRemove.length; ++r) {
            try {
              ...
              if (mats) {
                var materials = Array.isArray(mats) ? mats : [mats];

                for (var m = 0; m < materials.length; ++m) {
                  var textureMap = materials[m].map;
    ...
    _createClass(AssetViewer, [{
      ...
      */
      ...
      value: function scaleModel() {
        ...
        var boundingBox = new THREE$1.Box3().setFromObject(this.resource);
